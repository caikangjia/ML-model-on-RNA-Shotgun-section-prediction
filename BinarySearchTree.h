...
private:
  ...
  static int height_impl(const Node *node) {
    if (node == nullptr){return 0;}
    return (int)std::max(height_impl(node->left), height_impl(node->right)) + 1;
    assert(false);
  }
  ...
  static Node * insert_impl(Node *node, const T &item, Compare less) {
	  ...
	  // if the item is smaller than current node, add it to the left
	  if (less(item, node->datum)) {
		  // if there is no more node on left, add one
		  if (!node->left)
			  node->left = insert_impl(node->left, item, less);
		  else
			  insert_impl(node->left, item, less);
	  }
	  // if the item is larger than current node, add it to the right
	  else {		  
		  if (!node->right)
			  node->right = insert_impl(node->right, item, less);
		  // if there isn't a right node, set the node to be root
		  else
  ...
  static bool check_sorting_invariant_impl(const Node *node, Compare less) {
    ...
    if (node->left){
      if (!less(max_element_impl(node->left)->datum, node->datum)){return false;} // PAY ATTENTION THAT IT COULD BE EQUAL!!! SO YOU CAN'T WRITE LIKE 'NOT LESS'.
      //if (not less(max_element_impl(node->left)->datum, node->datum)){return false;}
    }
    if (node->right){
      if (!less(node->datum, min_element_impl(node->right)->datum)){return false;}
    }
  ...
  static void traverse_preorder_impl(const Node *node, std::ostream &os) {
    ...
    traverse_preorder_impl(node->right, os); // PRE!!! I copied and forgot to change the inorder yto preorder.
    return;
    assert(false);
  }
  ...
  static Node * min_greater_than_impl(Node *node, const T &val, Compare less) {
	  if (!node)
		  return node;
	  // if current node isn't larger than val, move to right
	  if (!less(val, node->datum)) {
		  ...
		  // if there is not more left nodes, stop and return current node
		  if (!node->left)
			  return node;
		  // if there is a left node, and the left node is larger than val
		  // keep tracking down
		  if (less(val, node->left->datum))
			  return min_greater_than_impl(node->left, val, less);
		  // left node isn't less than val, return current
		  else
			  return node;
	  }
  }
